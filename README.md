...
- AUTOMORPH_BATCH_SIZE : batch size for the M1 quality ensemble. The default is 64. Lower it if you hit a memory error (see below).
- AUTOMORPH_VESSEL_THRESHOLD : probability above which M2 calls a pixel vessel. The default is 0.2. Raise it towards 0.5 for a more conservative segmentation. Benchmarked against the FIVES expert annotations, 0.2 is the Dice optimum and 0.5 costs about 0.03 Dice and 0.12 sensitivity — see [benchmark/docs/vessel_threshold.md](benchmark/docs/vessel_threshold.md).
